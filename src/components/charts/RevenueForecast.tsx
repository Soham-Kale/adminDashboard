...
export function RevenueForecast({ data, isLoading, isError }: Props) {
  const forecast = data
    ? [
        ...data.map((d) => ({ ...d, type: "actual" as const, forecast: null })),
        ...[1, 2, 3].map((m) => {
    ...
    >
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={forecast} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
          ...
          <YAxis tickFormatter={(v: number) => `$${(v / 1000).toFixed(1)}k`} tick={{ fontSize: 10, fill: "hsl(240 5% 64.9%)" }} tickLine={false} axisLine={false} />
          <Tooltip formatter={(v: unknown) => v ? [`$${Number(v).toLocaleString()}`, "Revenue"] : ["N/A", ""]} contentStyle={{ background: "hsl(240 10% 5.9%)", border: "1px solid hsl(240 3.7% 15.9%)", borderRadius: 8, fontSize: 12 }} />

...
interface Props {
  data?: Array<{ month: string; revenue: Record<string, number> | number }>;
  isLoading?: boolean;
  isError?: boolean;
}
...
export function RevenueForecast({ data, isLoading, isError }: Props) {
  ...
  const forecast = data
    ? [
        ...data.map((d) => {
          const rev = typeof d.revenue === "object"
            ? pickRevenue(d.revenue, selectedCurrency)
            : (d.revenue ?? 0);
          return { ...d, revenue: rev, type: "actual" as const, forecast: null };
        }),
        ...[1, 2, 3].map((m) => {
          const lastRaw = data[data.length - 1]?.revenue ?? 0;
          const lastRevenue = typeof lastRaw === "object"
            ? pickRevenue(lastRaw, selectedCurrency)
            : (lastRaw ?? 0);
          ...
          return {
            ...
            revenue: null,
            forecast: Math.round(lastRevenue * Math.pow(1.08, m / 12) * (1 + m * 0.02)),
  ...
  return (
    <ChartWrapper
      ...
      description={`Actual (${selectedCurrency}) + 3-month forecast at 8% MoM growth`}
      isLoading={isLoading}
      isError={isError}
    ...
    >
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={forecast} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
          ...
          <Tooltip
            formatter={(v: unknown) => v ? [`${selectedCurrency} ${Number(v).toLocaleString()}`, "Revenue"] : ["N/A", ""]}
          ...
          <ReferenceLine
            ...
            strokeDasharray="4 2"
            label={{ value: "Today", fill: "hsl(240 5% 64.9%)", fontSize: 10 }}
          />
